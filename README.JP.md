...
- `--update-traffic`：タグ付け後にそのリビジョンにトラフィックを向けます
- `--remove-tags`：新しいタグを作成する前に既存のすべてのリビジョンタグを削除します
...
- **タイムアウトエラー**：デフォルトでは、Dekopinのタイムアウトは120秒です。長時間実行される操作では、コード内でこの値を増やすことを検討してください。
